...
export const addToCart = async (req, res) => {
  try {
    ...
    const { userid, mobileNumber, type, price, discount, estimatedTime, image, name, reviews, slotBookedDate, slotBookedTime, technology, time, totalPrice, warranty, address } = req.body;

    // Basic validation
    if (!userid || !mobileNumber || !type || !price || !address) {
      return res.status(400).json({ error: 'Userid, mobile number, type, price, and address are required' });
    }
